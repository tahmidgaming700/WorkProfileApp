...
public partial class MainForm : Form
{
    ...
    private async void btnCheckUpdate_Click(object? sender, EventArgs e)
    {
        try
        {
            ...
            lblBuildRange.Text = $"Build transition: {_manifest.CurrentBuild ?? "26200.7080"} → {_manifest.TargetBuild ?? "26200.8080"}";

            txtChangelog.Text = "Fetching Microsoft changelog...";
            txtAiSummary.Text = "Generating AI summary...";

            if (!string.IsNullOrWhiteSpace(_manifest.ChangelogUrl))
            {
                var changelogText = await _updater.FetchChangelogTextAsync(_manifest.ChangelogUrl, CancellationToken.None);
                txtChangelog.Text = changelogText.Length > 4000
                    ? changelogText[..4000] + "..."
                    : changelogText;

                var summary = _updater.GenerateAiSummary(
                    changelogText,
                    _manifest.CurrentBuild ?? "26200.7080",
                    _manifest.TargetBuild ?? "26200.8080");
                txtAiSummary.Text = summary;
            }
            else
            {
                txtChangelog.Text = "No changelogUrl in manifest. Add a Microsoft release-notes URL to display it here.";
                txtAiSummary.Text = "AI summary unavailable: changelog URL is missing in manifest.";
            }
        ...
        catch (Exception ex)
        {
            ...
            txtChangelog.Text = "Failed to fetch changelog.";
            txtAiSummary.Text = "AI summary unavailable due to update-check failure.";
